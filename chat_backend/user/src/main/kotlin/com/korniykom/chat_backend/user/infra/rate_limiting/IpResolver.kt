package com.korniykom.chat_backend.user.infra.rate_limiting

import com.korniykom.chat_backend.user.infra.config.NginxConfig
...
class IpResolver(
    ...
    private val trustedMatchers: List<IpAddressMatcher> =
        nginxConfig.trustedIps.filter { it.isNotBlank() }.map { proxy ->
            val cidr = when {
    ...
    fun getClientIp(request: HttpServletRequest): String {
        ...
        if(!isFromTrustedProxy(remoteAddr)) {
            if(nginxConfig.requiredProxy) {
                logger.warn("Dirrect connection attempt from $remoteAddr")
    ...
    private fun extractFromXRealIp(
        ...
        return request.getHeader("X-real-IP")?.let { header ->
            validateAndNormilizeIp(header, "X-real-IP", proxyIp)
        }
    }

    private fun validateAndNormilizeIp(ip: String, headerName: String, proxyIp: String ): String? {
